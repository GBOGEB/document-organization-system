...
class ControlPlaneV05Tests(unittest.TestCase):
    ...
    def test_telemetry_is_measured_and_covers_entity_classes(self):
        ...
        self.assertEqual(len([r for r in s["rows"] if r["entity_type"]=="ENTRY"]),7)
        self.assertGreater(s["global_control_events"],0)

    def test_global_receipts_are_not_multiplied_into_entities(self):
        rows=telemetry.measured_rows(); scoped=[r for r in rows if r["entity_type"] in {"ENTRY","BOOK","OPERATOR"}]
        self.assertTrue(all(r["runtime_proof_count"]==0 for r in scoped)); self.assertTrue(all(r["promotion_count"]==0 for r in scoped))

    def test_pca_has_pc1_pc2_and_measured_ranking(self):
        r=pca_priority.report(); self.assertEqual(r["basis"],"MEASURED_REPOSITORY_TELEMETRY")
    ...
    def test_shared_and_local_rebuild_blast_radius(self):
        ...
        self.assertEqual(glob_runtime.plan_rebuild(["ATOM-LKT-INVCOP"])["affected_entries"],["ENTRY-LKT-INVCOP"])
        self.assertEqual(glob_runtime.plan_rebuild(["ATOM-LKT-QEQ"])["affected_entries"],["ENTRY-LKT-PERFORMANCE-POINT"])
        self.assertEqual(glob_runtime.plan_rebuild(["ATOM-LKT-DIRECT-SUBTOTAL"])["affected_entries"],["ENTRY-LKT-POWER-BREAKDOWN"])
        self.assertEqual(glob_runtime.plan_rebuild(["ATOM-CODEX-RECEIPT"])["affected_entries"],["ENTRY-LKT-RUNTIME-RECEIPT"])
    ...
    def test_worker_plan_uses_priority_and_measured_cost(self):
        ...
        self.assertEqual(p["mode"],"PLAN_ONLY"); self.assertEqual(p["capacity"],2); self.assertEqual(len(p["assignments"]),3)
    ...
    def test_ask_runtime_reaches_current_control_chain(self):
        out=glob_runtime.ask_runtime("ENTRY-LKT-INVCOP"); versions={e["version"] for e in out["proofs"]+out["promotions"]}
        self.assertIn("Synchronized Views v0.4",versions); self.assertTrue(all(len(e["sha"])==40 and e["run_id"]>0 for e in out["proofs"]+out["promotions"]))

    def test_provenance_expands_monotonically(self):
        a=glob_runtime.ask_provenance("ENTRY-LKT-INVCOP","summary"); b=glob_runtime.ask_provenance("ENTRY-LKT-INVCOP","evidence"); c=glob_runtime.ask_provenance("ENTRY-LKT-INVCOP","runtime")
        self.assertEqual(a["target"],b["target"]); self.assertEqual(b["target"],c["target"]); self.assertIn("runtime",c); self.assertNotIn("runtime",a)
