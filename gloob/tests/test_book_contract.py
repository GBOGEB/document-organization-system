...
class GloobBookTests(unittest.TestCase):
    def setUp(self):
        ...
        self.performance_path=ROOT/"graph"/"qplant-energy-performance.json"
        self.power_path=ROOT/"graph"/"qplant-energy-power.json"
        self.evidence_path=ROOT/"graph"/"qplant-energy-evidence.json"
        self.runtime_path=ROOT/"graph"/"qplant-energy-runtime.json"
    ...
    def test_depth_projection_is_monotonic_across_decomposed_family(self):
        graphs=[rb.load_graph(p)[0] for p in (self.graph_path,self.performance_path,self.power_path,self.evidence_path,self.runtime_path)]
    ...
    def test_decomposed_entries_render_without_atom_copying(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('<a id="lkt-invcop"></a>',md); self.assertNotIn('<a id="equivalent-load"></a>',md); self.assertNotIn('<a id="sat-metering-boundary"></a>',md)
            self.assertIn("### LKT invCOP offered point",md); self.assertIn("GBOGEB/cryoplant-project@a9cb3241",md)

            performance=rb.render_all(self.performance_path,root/"performance","deep")
            performance_md=Path(performance["outputs"]["md"]["path"]).read_text()
            self.assertIn('<a id="equivalent-load"></a>',performance_md); self.assertIn('<a id="total-power"></a>',performance_md)

            evidence=rb.render_all(self.evidence_path,root/"evidence","deep")
            evidence_md=Path(evidence["outputs"]["md"]["path"]).read_text()
            self.assertIn('<a id="sat-metering-boundary"></a>',evidence_md); self.assertNotIn("GBOGEB/CODEX@631bc364",evidence_md)

            runtime=rb.render_all(self.runtime_path,root/"runtime","deep")
            runtime_md=Path(runtime["outputs"]["md"]["path"]).read_text()
            self.assertIn("GBOGEB/CODEX@631bc364",runtime_md)

            families=[self.graph,rb.load_graph(self.performance_path)[0],rb.load_graph(self.power_path)[0],rb.load_graph(self.evidence_path)[0],rb.load_graph(self.runtime_path)[0]]
            seen=set()
            for graph in families:
                ids={a["id"] for a in graph["atoms"]}; self.assertTrue(seen.isdisjoint(ids)); seen |= ids
