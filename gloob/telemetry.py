...
def events_for(entry_id, timeline):
    return [e for e in timeline.get("events",[]) if "*" in e.get("entry_ids",[]) or entry_id in e.get("entry_ids",[])]
...
def measured_rows():
    ...
    for eid,g in graphs.items():
        ...
        for op in g.get("operators",[]): op_entries.setdefault(op["id"],set()).add(eid)
        ev = events_for(eid,timeline)
        ...
        rows.append(dict(entity_id=eid,entity_type="ENTRY",book_id=recs[eid]["book"]["id"],
            ...
            promotion_count=sum(e["kind"]=="PROMOTION" for e in ev),operator_count=len(g.get("operators",[])),cross_ref_count=len(xrefs)))
    ...
    for book in registry.get("books",[]):
        ...
        for eid in eids:
            ...
            ops |= {o["id"] for o in g.get("operators",[])}; edge_count += len(g.get("edges",[])); source_count += len(g.get("sources",[])); ev += events_for(eid,timeline)
    ...
    for op,eids in sorted(op_entries.items()):
        ev=[x for eid in eids for x in events_for(eid,timeline)]
        rows.append(dict(entity_id=op,entity_type="OPERATOR",book_id="MULTI",structural_degree=len(eids),scope_size=len(eids),reuse_count=len(eids),source_count=0,
            runtime_proof_count=sum(e["kind"]=="RUNTIME_PROOF" for e in ev),promotion_count=sum(e["kind"]=="PROMOTION" for e in ev),operator_count=len(eids),cross_ref_count=0))
    return sorted(rows,key=lambda r:(r["entity_type"],r["entity_id"]))

def snapshot():
    return {"schema":"gloob-measured-telemetry/0.1","expert_seeded_scores":False,"features":FEATURES,"rows":measured_rows()}
